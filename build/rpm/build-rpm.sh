#!/bin/bash
# Script de création package .rpm pour Tuxpilot - VERSION FINALE
# Usage: cd build/rpm && ./build-rpm.sh

set -e  # Arrêter si erreur

echo "🚀 Building Tuxpilot .rpm package (sans EventPipe/lttng)..."
...
echo -e "${BLUE}📦 Étape 2/6 : Publication .NET (sans EventPipe/lttng)${NC}"
INSTALL_ROOT="$BUILD_DIR/BUILDROOT/$APP_NAME-$APP_VERSION"
mkdir -p "$INSTALL_ROOT/opt/tuxpilot"

# ✅ DÉSACTIVER EventPipe pour éviter dépendance lttng-ust
...
# Désactiver EventPipe au runtime
export COMPlus_EnableEventPipe=0
cd /opt/tuxpilot
exec ./Tuxpilot.UI "$@"
EOF
...
# Date au format anglais pour RPM
...
Requires:       python3, python3-requests, python3-psutil
...
echo "  - Upload RPM + install-tuxpilot.sh sur ton site"
echo "  - Compatible : Fedora 41-43, RHEL 9+, Rocky Linux 9+"
echo ""
echo "ℹ️  EventPipe désactivé pour éviter dépendance lttng-ust"
echo "ℹ️  Dépendances : python3, python3-requests, python3-psutil"
