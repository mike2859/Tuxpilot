...
/// <summary>
/// Service de planification utilisant systemd timers
/// </summary>
public class ServicePlanificationSystemd : IServicePlanification
{
    private readonly ExecuteurScriptPython _executeur;
    private const string SCRIPT_NAME = "planification_systemd.py";

    public ServicePlanificationSystemd(ExecuteurScriptPython executeur)
    {
    ...
    public async Task<List<TachePlanifiee>> ObtenirTachesAsync()
    {
        try
        {
            ...
            {
                Console.WriteLine("[SYSTEMD] Aucun résultat");
        ...
        {
            Console.WriteLine($"[SYSTEMD] Erreur obtention tâches : {ex.Message}");
            return new List<TachePlanifiee>();
        }
    }
    ...
    public async Task<bool> AjouterTacheAsync(TachePlanifiee tache)
    {
        try
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(jsonResult))
                {
                    Console.WriteLine("[SYSTEMD] Résultat vide");
                    return false;
                }
                ...
                if (root.TryGetProperty("success", out var success))
                {
                    ...
                    Console.WriteLine($"[SYSTEMD] Résultat ajout: {result}");
                    return result;
                }
        ...
        {
            Console.WriteLine($"[SYSTEMD] Erreur ajout : {ex.Message}");
            return false;
        }
    }
    ...
    public async Task<bool> SupprimerTacheAsync(string id)
    {
        ...
        {
            Console.WriteLine($"[SYSTEMD] Erreur suppression : {ex.Message}");
            return false;
        }
    }
    ...
    public async Task<bool> ToggleTacheAsync(string id)
    {
        ...
        {
            Console.WriteLine($"[SYSTEMD] Erreur toggle : {ex.Message}");
            return false;
        }
    }
    ...
    private TachePlanifiee? ParseTache(JsonElement json)
    {
        ...
        {
            Console.WriteLine($"[SYSTEMD] Erreur parsing tâche : {ex.Message}");
            return null;
        }
    }
}